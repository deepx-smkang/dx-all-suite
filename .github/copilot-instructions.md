# DX All Suite — Copilot Global Instructions

> This is the top-level repository for the DEEPX All Suite (DX-AS). It contains
> the dx-runtime submodule, which in turn contains dx_app and dx_stream submodules.
...
When working on code, navigate to the appropriate submodule:

- **Standalone inference**: Open `dx-runtime/dx_app/` — has its own agents, skills, and instructions
- **GStreamer pipelines**: Open `dx-runtime/dx_stream/` — has its own agents, skills, and instructions
- **Cross-project integration**: Open `dx-runtime/` — handles unified validation and build orchestration
- **Model compilation**: Open `dx-compiler/` — has its own agents, skills, and instructions
- **Cross-suite** (compile + deploy): Orchestrate across `dx-compiler/` and `dx-runtime/`

Each submodule has its own `.github/copilot-instructions.md` with full context.
...
Use `@agent-name` in Copilot Chat to invoke these agents:
...
MUST be invoked** (`@dx-compiler-builder` or `@dx-dxnn-compiler`) and the actual
`dxcom` command MUST be executed. Never generate placeholder compilation scripts or
stub code — produce the real `.dxnn` file.
...
iter-8). Read the R31 Session Layout HARD GATE in the compiler agent document
...
`.github/copilot-instructions.md` for extended context (model registry, import
patterns, etc.). But the rules below are sufficient for correct code generation
even without reading sub-project files.
