...
모든 AI 생성 파일은 대상 서브 프로젝트 내 `dx-agent-dev/<session_id>/`에
...
1. **Compiler session** → `dx-compiler/dx-agent-dev/<session_id>/` (`compile.py`,
   `config.json`, `yolo26n.dxnn`, `verify.py`, `session.log` 포함)
2. **App session** → `dx-runtime/dx_app/dx-agent-dev/<session_id>/` (factory,
...
(`dx-agent-dev/<session_id>/`)에 다음 파일을 생성해야 합니다. 이것은
...
8. **출력 격리** — 모든 생성된 코드는 `dx-agent-dev/<session_id>/`에 있어야
