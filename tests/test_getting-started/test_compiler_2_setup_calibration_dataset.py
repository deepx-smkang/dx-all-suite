...
def test_compiler_2_setup_calibration_dataset():
    result = run_command(["bash", str(GETTING_STARTED_DIR / "compiler-2_setup_calibration_dataset.sh")], banner_msg="Running script: compiler-2_setup_calibration_dataset.sh", timeout=DEFAULT_TIMEOUT, cwd=GETTING_STARTED_DIR)
