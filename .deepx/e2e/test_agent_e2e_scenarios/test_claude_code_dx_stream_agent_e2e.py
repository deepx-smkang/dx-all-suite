...
"""
Agent-Driven E2E Test (Claude Code): dx_stream Scenario — Detection Pipeline with Tracking
...
This is the Claude Code counterpart of ``test_dx_stream_agent_e2e.py`` (Copilot CLI)
...
class TestMandatoryArtifacts:
    ...
    def test_session_txt_export_exists(self, scenario: ScenarioResult):
        """Session transcript (session.txt) should be present for Claude Code."""
        if scenario.output_dir is None:
            pytest.skip("No output directory detected")
        # Claude Code exports as .txt; the harness copies it as session.txt.
    ...
    def test_session_txt_in_correct_dir(self, scenario: ScenarioResult):
        """R17: session.txt should be written to Claude Code's output directory by the runner."""
        if scenario.output_dir is None:
            pytest.skip("No output directory detected")
