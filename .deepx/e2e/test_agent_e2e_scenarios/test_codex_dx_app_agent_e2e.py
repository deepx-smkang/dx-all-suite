...
"""
Agent-Driven E2E Test (Codex): dx_app Scenario — Build a Python Detection App

Runs the Codex CLI inside dx_app/ with a prompt that requests a
...
This is the Codex counterpart of ``test_dx_app_agent_e2e.py`` (Copilot CLI)
...
def scenario(codex_runner, app_codex_cli_artifacts_dir) -> ScenarioResult:
    """Execute dx_app Scenario via Codex CLI."""
    return codex_runner.run(
        prompt=SCENARIO_PROMPT,
        workdir=APP_ROOT,
        scenario_key="dx_app",
        session_log_dir=app_codex_cli_artifacts_dir,
