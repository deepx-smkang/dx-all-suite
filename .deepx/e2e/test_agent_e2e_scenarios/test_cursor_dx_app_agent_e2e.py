...
"""
Agent-Driven E2E Test (Cursor CLI): dx_app Scenario — Build a Python Detection App

Runs the Cursor CLI ``agent`` inside dx_app/ with a prompt that requests a
...
This is the Cursor CLI counterpart of ``test_dx_app_agent_e2e.py`` which
uses Copilot CLI.  Both share the same assertions; only the runner differs.
"""
...
def scenario(cursor_runner, app_cursor_cli_artifacts_dir) -> ScenarioResult:
    """Execute dx_app Scenario via Cursor CLI."""
    return cursor_runner.run(
        prompt=SCENARIO_PROMPT,
        workdir=APP_ROOT,
        scenario_key="dx_app",
        session_log_dir=app_cursor_cli_artifacts_dir,
