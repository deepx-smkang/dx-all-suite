...
"""
Agent-Driven E2E Test (Codex): dx_stream Scenario — Detection Pipeline with Tracking

Runs the Codex CLI inside dx_stream/ with a prompt requesting a
detection pipeline using yolo26n with tracking.  Verifies GStreamer elements.

This is the Codex counterpart of ``test_dx_stream_agent_e2e.py`` (Copilot CLI)
and ``test_cursor_dx_stream_agent_e2e.py`` (Cursor CLI).
"""
...
pytestmark = [
    pytest.mark.agent_e2e_codex_cli_autopilot,
]
...
def scenario(codex_runner, stream_codex_cli_artifacts_dir) -> ScenarioResult:
    """Execute dx_stream Scenario via Codex CLI."""
    with _apt_lock():
        return codex_runner.run(
            prompt=SCENARIO_PROMPT,
            workdir=STREAM_ROOT,
            scenario_key="dx_stream",
            session_log_dir=stream_codex_cli_artifacts_dir,
        )


class TestExecution:
    """Codex CLI execution basics."""
    ...
    def test_session_log_saved(self, scenario: ScenarioResult):
        """Session transcript is saved as Codex JSONL session output."""
        if not scenario.succeeded:
            pytest.skip("Codex execution failed — skipping log check")
...
class TestMandatoryArtifacts:
    ...
    def test_session_txt_export_exists(self, scenario: ScenarioResult):
        """Session transcript (session.txt) should be present for Codex."""
        if scenario.output_dir is None:
            pytest.skip("No output directory detected")
        # Codex exports as .txt; the harness copies it as session.txt.
        # Also accept session.html in case a future harness update changes format.
    ...
    def test_session_txt_in_correct_dir(self, scenario: ScenarioResult):
        """R17: session.txt should be written to Codex's output directory by the runner."""
        if scenario.output_dir is None:
            pytest.skip("No output directory detected")
        session_txt = scenario.output_dir / "session.txt"
        assert session_txt.exists(), (
            f"session.txt missing from {scenario.output_dir} — "
            "check CodexRunnerAutopilot.run() output_dir resolution (R17)"
        )
