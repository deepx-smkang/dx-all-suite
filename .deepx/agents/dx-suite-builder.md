...
description: >
  Top-level builder for DEEPX All Suite. Routes tasks to the appropriate
  submodule (dx_app, dx_stream, dx-runtime, or dx-compiler) based on task
  classification. Handles cross-suite orchestration (compile + deploy).
...
routes-to:
  - label: Compile model
    target: dx-compiler-builder
    description: Route compilation tasks to dx-compiler
  - label: Build runtime app
    target: dx-runtime-builder
    description: Route runtime/app/stream tasks to dx-runtime
---
